 ...
 */
import {
  getPiece, putPiece, getCatalog, putCatalog, deletePieceFromBlob,
...
/**
 * Remove a piece from storage. Deletes any Blob objects under
 * pieces/{slug}/ and drops the catalog entry. Disk-only pieces (the
 * git-committed ones) can't be physically removed from this code path,
 * but pulling them from the catalog hides them from the hub — the
 * practical "delete" the user wants. Direct /<slug>/ URLs continue to
 * work for those via the disk fallback in api/deck.js.
 */
export async function removeItem({ slug }) {
  ...
  const blobResult = await deletePieceFromBlob(slug);

  let catalogUpdated = false;
  try {
    const catalog = await getCatalog();
    const before = catalog.pieces || [];
    const after = before.filter(p => p.slug !== slug);
    if (after.length !== before.length) {
      await putCatalog({ pieces: after });
      catalogUpdated = true;
    }
  } catch (e) {
    console.warn('[item-mutate] Catalog delete failed:', e.message);
  }

  return {
    ok: true,
    slug,
    blob_deleted: blobResult.deleted || 0,
