...
const deck1 = z.object({
  ...
  // Decks are finished/in-progress presentations; templates are reusable
  // starting points. Same file format, same routes — `kind` distinguishes
  // them in the catalog and the hub IA. Defaults to "deck" so pre-existing
  // files without the field continue to load as decks.
  kind: z.enum(['deck', 'template']).default('deck'),
  // Slug of the source template, when this piece was derived from one;
  // null otherwise.
  parent_template: z.string().regex(slugPattern).nullable().default(null),
  // Whether the piece is pinned to the hub's persistent sidebar.
  // Toggled post-upload via /api/item-update; defaults false so older
  // files load unpinned.
