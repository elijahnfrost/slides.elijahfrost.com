...
export async function runUpload({ rawHtml, origin, allowedEmbedHosts, kindOverride }) {
  ...
  if (!ogBuffer && ogStatus !== 'failed') ogStatus = 'skipped';
  ...
  try {
    ...
    // Uploading a slug clears any tombstone for it — the user is
    // resurrecting it deliberately.
    const tombstones = (catalog.tombstones || []).filter(s => s !== newMeta.slug);
