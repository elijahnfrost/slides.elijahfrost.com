 ...
 *
 * Steps:
 *  1  Auth (DECK_UPLOAD_TOKEN)
 *  2  Rate limit (handled at api/ layer)
 *  3  Size cap (2 MB)
 *  4  HTML parse
 *  5  Extract deck-meta
 *  6  Migration chain
 *  7  Framework version known
 *  8  Framework fences match canonical
 *  9  Optimistic concurrency (parent_version_id)
 * 10  Structural validation
 * 12  Mint version_id + timestamps
 * 13  Render OG image (best-effort)
 * 14  Store to Vercel Blob + update catalog
 * 15  Return success
 */
...
export async function runUpload({ rawHtml, authHeader, origin, allowedEmbedHosts }) {
  ...
  const onDisk = await getPiece(meta.slug);
  if (onDisk) {
  ...
  // Step 13 — render OG image (best-effort)
  let ogBuffer = null;
  try {
  ...
  // Step 14 — store to Vercel Blob + update catalog incrementally
  try {
